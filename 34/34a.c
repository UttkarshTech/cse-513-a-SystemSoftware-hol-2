/*
============================================================================
Name : 34a.c
Author : Uttkarsh Ranjan
Description :34. Write a program to create a concurrent server.
                a. use fork
                b. use pthread_create
Date: 22nd September 2025
============================================================================
*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <arpa/inet.h>
#include <sys/types.h>
#include <sys/socket.h>

#define SERVER_PORT 8080
#define BUFFER_LEN 1024
#define BACKLOG 5

int create_server_socket() {
    int sock_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (sock_fd < 0) {
        perror("Socket creation failed");
        exit(EXIT_FAILURE);
    }
    return sock_fd;
}

void bind_server_socket(int sock_fd, int port, struct sockaddr_in *addr) {
    addr->sin_family = AF_INET;
    addr->sin_addr.s_addr = INADDR_ANY;
    addr->sin_port = htons(port);

    if (bind(sock_fd, (struct sockaddr *)addr, sizeof(*addr)) < 0) {
        perror("Bind failed");
        exit(EXIT_FAILURE);
    }
}

void listen_for_connections(int sock_fd, int backlog) {
    if (listen(sock_fd, backlog) < 0) {
        perror("Listen failed");
        exit(EXIT_FAILURE);
    }
    printf("Server listening on port %d...\n", SERVER_PORT);
}

void handle_client(int client_fd) {
    char buffer[BUFFER_LEN] = {0};
    read(client_fd, buffer, BUFFER_LEN);
    printf("Message from client: %s\n", buffer);

    char *reply = "Hello from server (fork)";
    send(client_fd, reply, strlen(reply), 0);

    close(client_fd);
    exit(0);
}

int main() {
    int server_fd = create_server_socket();
    struct sockaddr_in server_addr;
    bind_server_socket(server_fd, SERVER_PORT, &server_addr);
    listen_for_connections(server_fd, BACKLOG);

    while (1) {
        struct sockaddr_in client_addr;
        socklen_t addr_len = sizeof(client_addr);
        int client_fd = accept(server_fd, (struct sockaddr *)&client_addr, &addr_len);
        if (client_fd < 0) {
            perror("Accept failed");
            continue;
        }

        pid_t pid = fork();
        if (pid < 0) {
            perror("Fork failed");
            close(client_fd);
            continue;
        }

        if (pid == 0) { // child process
            printf("Child %d handling this client\n", getpid());
            close(server_fd);
            handle_client(client_fd);
        } else { // parent process
            close(client_fd);
        }
    }

    close(server_fd);
    return 0;
}

/*
---SERVER---
uttkarsh-ranjan@uttkarsh-ranjan-ubuntu-vm:~/Desktop/SystemSotware/HOL-2/34$ gcc 34a.c
uttkarsh-ranjan@uttkarsh-ranjan-ubuntu-vm:~/Desktop/SystemSotware/HOL-2/34$ gcc 34_client.c -o 34_client.out
uttkarsh-ranjan@uttkarsh-ranjan-ubuntu-vm:~/Desktop/SystemSotware/HOL-2/34$ ./a.out
Server listening on port 8080...

---CLIENT 1---
uttkarsh-ranjan@uttkarsh-ranjan-ubuntu-vm:~/Desktop/SystemSotware/HOL-2/34$ ./34_client.out

---SERVER---
Child 14208 handling this client

---CLIENT 2---
uttkarsh-ranjan@uttkarsh-ranjan-ubuntu-vm:~/Desktop/SystemSotware/HOL-2/34$ ./34_client.out

---SERVER---
Child 14211 handling this client

---CLIENT 1---
Message sent to server: Hello from client!

---SERVER---
Message from client: Hello from client!

---CLIENT 2---
Message sent to server: Hello from client!

---SERVER---
Message from client: Hello from client!

---CLIENT 1---
Response from server: Hello from server (fork)

---CLIENT 2---
Response from server: Hello from server (fork)

*/
